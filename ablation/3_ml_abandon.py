"""ABLATION: Use ML model for abandon rate instead of historical averages."""
from shared import *
...
for p in PORTFOLIOS:
    ...
    for met in ['Call Volume','CCT']:
        ...
        gr=g25/g24 if g24>0 else 1.0;bl=np.zeros(31)
        ...
        preds[p][met]=0.7*ml[:31]+0.3*bl
    # CHANGE: ML model for abandon rate
    ytr=cl.loc[trn,'tgt_Abandon Rate'].values;yv=cl.loc[val,'tgt_Abandon Rate'].values
    gb2=HistGradientBoostingRegressor(loss='squared_error',max_iter=500,max_depth=4,learning_rate=0.03,min_samples_leaf=20,random_state=42)
    gb2.fit(Xtr,ytr);rd2=Ridge(alpha=1.0);rd2.fit(np.nan_to_num(Xtr,0),ytr)
    vp_abd=0.6*gb2.predict(Xv)+0.4*rd2.predict(np.nan_to_num(Xv,0))
    print(f'  {p} ABD val MAE: {mean_absolute_error(yv,vp_abd):.4f} (actual avg: {yv.mean():.4f})')
    amsk=(ft['Date']>='2025-08-01')&(ft['Date']<='2025-08-31')
    Xa=ft.loc[amsk,cols].fillna(method='ffill').fillna(method='bfill').fillna(0)
    abd=0.6*gb2.predict(Xa.values)+0.4*rd2.predict(np.nan_to_num(Xa.values,0))
    abd=np.clip(abd[:31],0,0.25)
    preds[p]['Abandon Rate']=abd
    print(f'  {p} ABD aug avg: {abd.mean():.4f}')
...
print(f"\n>> ML ABANDON RATE (instead of historical)")
for p in PORTFOLIOS:print(f"   {p}: ratio={ratios[p]:.3f}, abd_rate={abd_rates[p]:.4f}")
