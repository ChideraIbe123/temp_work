"""FULL pipeline (v4 as-is) — baseline for comparison."""
from shared import *
...
preds = {}
for p in PORTFOLIOS:
    ft = feats[p]; cols = feat_cols(ft)
    ok = ft[cols].notna().all(axis=1); cl = ft[ok].copy()
    ...
    for met in ['Call Volume','CCT']:
        ...
        gb.fit(Xtr,ytr)
        rd = Ridge(alpha=1.0); rd.fit(np.nan_to_num(Xtr,0),ytr)
    ...
    abd*=1.1; abd=np.clip(abd,0.002,0.25)
    preds[p]['Abandon Rate']=abd

res={p:{'cv':[],'abd':[],'ar':[],'cct':[]} for p in PORTFOLIOS}
for p in PORTFOLIOS:
    dcv,dcct,dar=preds[p]['Call Volume'],preds[p]['CCT'],preds[p]['Abandon Rate']
    dabd=dcv*dar
    for i,dt in enumerate(aug):
        dw=dt.dayofweek; cv=dcv[i]*cv_prof[p][dw]; ab=dabd[i]*abd_prof[p][dw]
        sc=dcct[i]/prof_cct_avg[p] if prof_cct_avg[p]>0 else 1.0
        cc=cct_prof[p][dw]*sc; ar=np.where(cv>0,ab/cv,0.0)
        res[p]['cv'].append(cv); res[p]['abd'].append(ab); res[p]['ar'].append(ar); res[p]['cct'].append(cc)
    for k in res[p]: res[p][k]=np.array(res[p][k])

for p in PORTFOLIOS:
    b=bias[p]; res[p]['cv']*=b; res[p]['abd']*=b; res[p]['cct']*=1.05
for p in PORTFOLIOS:
    res[p]['cv']=np.clip(res[p]['cv'],0,None); res[p]['abd']=np.clip(res[p]['abd'],0,None)
    res[p]['cct']=np.clip(res[p]['cct'],0,None)
    bad=res[p]['abd']>res[p]['cv']; res[p]['abd'][bad]=res[p]['cv'][bad]
    cv,ab=res[p]['cv'],res[p]['abd']
    res[p]['ar']=np.clip(np.where(cv>0,ab/cv,0.0),0,1)
    res[p]['cv']=np.round(cv).astype(int); res[p]['abd']=np.round(res[p]['abd']).astype(int)

ratios, abd_rates = score_submission(res, daily)
print(f"\n>> FULL (v4)")
for p in PORTFOLIOS:
    print(f"   {p}: ratio={ratios[p]:.3f}, abd_rate={abd_rates[p]:.4f}")
under = sum(1 for r in ratios.values() if r<1.0)
print(f"   Under-forecasting: {under}/4")
